...
from .constants import (CLIP_MEAN, CLIP_STD, IMAGENET_MEAN, IMAGENET_STD,
                        ...
                        SIGLIP_MEAN, SIGLIP_STD, MASK_CONTEXT_TOKEN, MASK_TOKEN,
...
def preprocess_internvl2_5(
    ...
    if has_mask:
        ...
        for conversation in conversations:
            if conversation['from'] == 'gpt':
                mask_cnt = conversation['value'].count('<mask>')
                for i in range(mask_cnt):
                    if current_mask_idx == num_mask:
                        break
                    mask_tokens = f'{DEFAULT_GE_START_TOKEN}{MASK_CONTEXT_TOKEN * (num_mask_token - 1)}{DEFAULT_GE_END_TOKEN}'
                    conversation['value'] = conversation['value'].replace('<mask>', mask_tokens, 1)
                    current_mask_idx += 1
            new_conversations.append(conversation)
        conversations = new_conversations
        assert current_mask_idx == num_mask, f'{current_mask_idx} != {num_mask}'
    ...
    gen_end_id = tokenizer.convert_tokens_to_ids(DEFAULT_GE_END_TOKEN)
    for role, input_id in zip(roles, input_ids):
        ...
        if role == 'system' or role == 'human':
            ...
            if gen_end_id in target:
                target[target==gen_end_id] = tokenizer.convert_tokens_to_ids(MASK_CONTEXT_TOKEN)
...
def vis_multi_mask(img_path, multi_masks, idx = 0, images_per_row = 4, total_images = 8):
    ...
    for i in [3,4,5,6,7,8,9]:
        cur_mask = multi_masks[i] # np.array | uint8 | 0-255
        cur_img = vis_mask(img, cur_mask)
        row = (i-2) // images_per_row
        col = (i-2) % images_per_row
        stitched_image.paste(cur_img, (col * image_width, row * image_height))

    # Save the stitched image
    output_path = f"results/{idx}-multi_mask.jpg"
