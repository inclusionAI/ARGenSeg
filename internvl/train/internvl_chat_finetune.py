...
from internvl.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      ...
                                      REF_START_TOKEN, MASK_TOKEN, 
                                      DEFAULT_GE_START_TOKEN, DEFAULT_GE_END_TOKEN,
                                      MASK_CONTEXT_TOKEN)
...
class CustomTrainer(Trainer):
    """Custom Trainer that logs text_loss and gen_loss."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._custom_loss_logged = True

    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        # Call the original method first
        result = super()._maybe_log_save_evaluate(tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval)

        # Add text_loss and gen_loss to logs if available
        if hasattr(model, 'text_loss') and model.text_loss is not None:
            text_loss_val = model.text_loss.item() if hasattr(model.text_loss, 'item') else float(model.text_loss)
            if model.gen_loss is not None:
                gen_loss_val = model.gen_loss.item() if hasattr(model.gen_loss, 'item') else float(model.gen_loss)
                logs = {'text_loss': round(text_loss_val, 4), 'gen_loss': round(gen_loss_val, 4)}
            else:
                logs = {'text_loss': round(text_loss_val, 4)}
            self.log(logs)
...
from internvl.model.var_vae.models.vqvae import VQVAEProcessor
from internvl.model.var_vae.models import VQVAE, build_vae
import imageio
...
class DataTrainingArguments:
    ...
    gen_img_reso: int = field(
        default=256,
        metadata={'help': 'VAE gen image resolution.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(f'[Dataset] pad2square: {pad2square}')
        ...
        self.vae_image_processor = VQVAEProcessor(final_reso=gen_img_reso, type='pad')
        self.vae_gen_processor = VQVAEProcessor(final_reso=gen_img_reso, type='square')
        with open("config/coco_class_names.json") as f:
            import json as json_load
            coco_class_name = json_load.load(f)
            self.coco_class_name = [kk for kk in coco_class_name]
    ...
    def multi_modal_get_item(self, data_item):
        ...
        pixel_values = torch.stack(pixel_values) # torch.Size([7, 3, 448, 448])

        has_mask = False
        if 'mask' in data_item:
            has_mask = True
            if type(data_item['mask']) == list:
                masks = [Image.open(mask).convert('RGB') for mask in data_item['mask']]
                for idx_mask in range(len(masks)):
                    mask = np.array(masks[idx_mask]).astype(np.uint8)
                    if mask.max() < 127: 
                        mask *=255
                    ...
                    masks[idx_mask] = mask
                mask_flags=torch.tensor([1] * len(masks), dtype=torch.long)
            elif "dataset" not in data_item or "refcoco" in data_item["dataset"]:
                mask = Image.open(data_item['mask']).convert('RGB')
                ...
                if not self.pad2square:
                    mask = mask.resize((512, 512))
                mask_flags=torch.tensor([1] * 1, dtype=torch.long)
                masks = [mask]
            else:
                sem_mask = imageio.imread(data_item['mask'])
                segments_info = data_item["seg_info"]
                # np.random.shuffle(segments_info)
                classes = [segment_info["category_id"] for segment_info in segments_info]
                if len(classes) == 0:
                    raise ValueError
                cur_cls = np.random.choice(np.unique(classes))
                cur_cls_name = self.coco_class_name[cur_cls]
                def pan_to_sem_mask(pan_mask, seg_infos):
                    sem_mask_ = np.ones_like(pan_mask)*255
                    for seg in seg_infos:
                        sem_mask_[pan_mask==seg['id']] = seg['category_id']
                    return sem_mask_
                mask = pan_to_sem_mask(sem_mask, segments_info)
                mask = (mask==cur_cls).astype(np.uint8)*255

                data_item['conversations'][0]['value'] = data_item["conversations"][0]['value'].format(cur_cls_name)
                mask = Image.fromarray(mask).convert("RGB")
                if not self.pad2square:
                    mask = mask.resize((512, 512))
                mask_flags=torch.tensor([1] * 1, dtype=torch.long)
            ...
            mask = Image.new('RGB', (224, 224), (255, 255, 255))
            if not self.pad2square:
                mask = mask.resize((512, 512))
            mask_flags=torch.tensor([0] * 1, dtype=torch.long)
            masks = [mask]
        # mask = self.vae_image_processor(mask)
        mask = [self.vae_image_processor(mask) for mask in masks]
        ...
        ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                  ...
                                  num_mask=len(masks), has_mask=has_mask)
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        if 'mask' in data_item:
            has_mask = True
            mask = Image.open(data_item['mask'][0]).convert('RGB')
            mask = np.array(mask).astype(np.uint8)
            if mask.max() < 127:
                mask *=255
            mask = Image.fromarray(mask).convert("RGB")
    ...
    def pure_text_get_item(self, data_item):
        ...
        if 'mask' in data_item:
            ...
            if np.random.rand() < 0.2:
                data_item['conversations'][0]["value"] = "Please generate a picture about nothing."
            # data_item['conversations'][1]["value"] = "The picture is as follows: <mask>"
        else:
            mask = Image.new('RGB', (224, 224), (255, 255, 255))
            mask_flags=torch.tensor([0] * 1, dtype=torch.long)
        mask = self.vae_gen_processor(mask)
        mask = torch.stack([mask])
        ...
        ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                  ...
                                  ds_name=self.ds_name, has_mask=has_mask)
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        dataset = LazySupervisedDataset(
            ...
            num_mask_token = model.reconstruct_tokens_num,
...
def main():
    ...
    token_list += [MASK_TOKEN, DEFAULT_GE_START_TOKEN, DEFAULT_GE_END_TOKEN, MASK_CONTEXT_TOKEN]
    ...
    visual_tokens_num = 4096
    new_tokens = ["<visual token {i:0>6d}>".format(i=i) for i in range(visual_tokens_num)]
    num_added_tokens = tokenizer.add_tokens(new_tokens, special_tokens=True)
    assert (num_added_tokens == visual_tokens_num or num_added_tokens == 0)
    num_new_tokens += num_added_tokens
    ...
    InternVLGenSeg.init_vae = False
    if model_args.model_name_or_path is not None:
        logger.info('Loading InternVLGenSeg...')
        ...
        logger.info('Building InternVLGenSeg...')
        model = InternVLGenSeg(internvl_chat_config, vision_model, llm)
    if "pretrained/InternVL2_5-8B" in model_args.model_name_or_path:
        model.init_seg_weights()
    model.img_context_token_id = img_context_token_id
    model.mask_context_token_id = mask_context_token_id
    model.gen_start_token_id = gen_start_token_id

    assert model.config.downsample_ratio == data_args.down_sample_ratio

    patch_nums=(1, 2, 3, 4, 5, 6, 8, 10, 13, 16) if data_args.gen_img_reso == 256 else (1, 2, 3, 4, 6, 9, 13, 18, 24, 32)
    vae = build_vae(
