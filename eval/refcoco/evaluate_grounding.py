...
class RefCOCODataset(torch.utils.data.Dataset):
    ...
    def __init__(self, test, prompt, input_size=224, dynamic_image_size=False,
        ...
        print(f"self.dynamic_image_size: {self.dynamic_image_size}")
        # self.transform = build_transform(is_train=False, input_size=input_size)
        self.transform = build_transform(is_train=False, input_size=input_size, pad2square=False)
...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        outputs = []
        for idx, (pixel_values, questions, bboxes, hws) in enumerate(tqdm(dataloader)):
        ...
        # print('*'*20 + f'{torch.distributed.get_rank()}: 3')
        ...
        if torch.distributed.get_rank() == 0:
            ...
            for i, output in enumerate(merged_outputs):
                ...
                try:
                    ...
                    # predict_bbox = (float(predict_bbox[0][0]), float(predict_bbox[0][1]), float(predict_bbox[0][2]),float(predict_bbox[0][3]))
                except:
                ...
                # if output['hw'][1] > output['hw'][0]:
                #     top_float = (output['hw'][1] - output['hw'][0])/output['hw'][1]/2
                #     predict_bbox[:, 1::2] -= top_float
                #     predict_bbox[:, 0::2] *= output['hw'][1]
                #     predict_bbox[:, 1::2] *= output['hw'][1]
                # else:
                #     left_float = (output['hw'][0] - output['hw'][1])/output['hw'][0]/2
                #     predict_bbox[:, 0::2] -= left_float
                #     predict_bbox[:, 0::2] *= output['hw'][0]
                #     predict_bbox[:, 1::2] *= output['hw'][0]
                predict_bbox[:, 0::2] *= max(output['hw'])
                predict_bbox[:, 1::2] *= max(output['hw'])
                predict_bbox[:, 0::2] -= (max(output['hw']) -output['hw'][1])/2
                predict_bbox[:, 1::2] -= (max(output['hw'])-output['hw'][0])/2
                # predict_bbox[:, 0::2] *= output['hw'][1]
                # predict_bbox[:, 1::2] *= output['hw'][0]
